...
`recon` is read-only recon with no mutation. `mechanical` is bounded scope,
named files, command-verifiable acceptance, no delegated judgment. `standard`
is bounded features/fixes with known patterns and local blast radius.
`expert` is open-ended shape, material ambiguity, system blast radius, a
hard-to-reverse change, or weak verification with mutation. `apex` is
foundational ambiguity, external-contract blast radius, destructive
